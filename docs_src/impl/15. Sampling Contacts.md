...
Agents' interactions in `epiworld` is implemented in various ways. Depending on the model, contacts will be drawn either directly from agents' connections (network models) or via sampling using a binomial distribution (mixing models). This section explains some of the optimizations implemented in the software to reduce computation time when modeling contacts between agents.

## Drawing contacts

In networked models, we assume that all contacts are drawn at each step of the simulation (a deterministic process). The randomness comes from the probability of transmission, which is given by the virus, the infectious agent, and susceptible agent. In the case of the mixing models (connected models where interaction happens as a function of a contact rate), we sample directly from the number of infected individuals. Mathemathically, the convolution of infected and not infected contacts distributes Binomial with paramters:

```math
X_\text{infected} + X_{\text{not infected}} \sim \text{Binomial}\left(n, \frac{c}{n}\right)
```

Where $X_\text{infected}\sim \text{Binomial}\left(n_\text{infected}, \frac{c}{n}\right)$. Since only infectious individuals (which we are refering to as infected) can change the status of susceptible individuals, the drawing process only draws individuals from the group of infectious individuals.
...
In the case of models featuring mixing matrices, the process unfolds similarly with the main difference that the overall number of contacts does not distribute Binomial. Particularly, given individual $i$ in group $g$, the number of contacts will be
...
Where the last equality follows from $A$ being row-stochastic. Like in the more general mixing models, the number of infected individuals to be drawn is $X(g,j)\sim\text{Binomial}\left(n(j)_\text{infected}, \frac{c}{n(j)}\times A(g, j)\right)$.

## Mixing models with quarantine

When the model features a quarantine process, or any other process that may affect the number of agents present in the model, the parameters of the Binomial distribution may change. In the case of the Measles models (`ModelMeaslesSchool`, `MeaslesMixing`, and `ModelMeaslesRiskQuarantine`), agents moved to the quarantine process change the contact probability, increasing the chances of contacting an infected individual. Looking again at the probability of contacting infected individuals:

```math
X(g,j)\sim\text{Binomial}\left(n(j)_\text{infected}, \frac{c}{n(j)}\times A(g, j)\right)
```

As $n(j)$, the total number of available agents in group $j$, decreases, the expected value of $X(g, j)$ will increase. This is especially true in those cases where the number of quarantined agents is large.
