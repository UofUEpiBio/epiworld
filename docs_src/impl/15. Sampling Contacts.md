...
Where $X_\text{infected}\sim \text{Binomial}\left(n_\text{infected}, \frac{c}{n}\right)$. Since only infectious individuals (which we are referring to as infected) can change the status of susceptible individuals, the drawing process only draws individuals from the group of infectious individuals. **This is an important distinction**, since sampling only from the number of infected individuals ensures that the overall contact rate is $c$ for infected individuals.
