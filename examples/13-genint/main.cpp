...
int main() {
    ...
    epiworld::epimodels::ModelSIRCONN<>
        model("avirus", popsize, 50.0 / popsize, r, p_i, p_r);
    ...
    double gentime_sir_mean = std::accumulate(
                                  gentime_sir.begin(),
                                  gentime_sir.begin() + max_days_for_calc,
                                  0.0
                              )
        / max_days_for_calc;
    ...
    epiworld::epimodels::ModelSEIRCONN<>
        model2("avirus", popsize, 50.0 / popsize, r, p_i, incubation, p_r);
    ...
    double gentime_seirconn_mean =
        std::accumulate(
            ...
            gentime_seirconn.begin() + max_days_for_calc,
            0.0
        )
        / max_days_for_calc;
    ...
    std::cout << "SIR Gen. Int. (obs)       : " << gentime_sir_obs_mean
              << std::endl;
    std::cout << "SIR Gen. Int. (expected)  : " << gentime_sir_mean
              << std::endl;

    std::cout << "SEIR Gen. Int. (obs)      : " << gentime_seir_obs_mean
              << std::endl;
    std::cout << "SEIR Gen. Int. (expected) : " << gentime_seirconn_mean
              << std::endl;
