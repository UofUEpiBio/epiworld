...
// (already included include/epiworld/math/lfmcmc/lfmcmc-bones.hpp)
...
#include <string>
// (already included include/epiworld/config.hpp)
// (already included include/epiworld/epiworld-macros.hpp)
// (already included include/epiworld/misc.hpp)
...
#include <fstream>
// (already included include/epiworld/config.hpp)
// (already included include/epiworld/adjlist-bones.hpp)
...
#include <unordered_map>
// (already included include/epiworld/config.hpp)
// (already included include/epiworld/userdata-bones.hpp)
// (already included include/epiworld/adjlist-bones.hpp)
// (already included include/epiworld/model-bones.hpp)
// (already included include/epiworld/virus-bones.hpp)
// (already included include/epiworld/agent-bones.hpp)
...
)
{
    ...
    if (probability_of_infecting)
        return probability_of_infecting(agent, *this, model);
...
)
{
    ...
    if (probability_of_recovery)
        return probability_of_recovery(agent, *this, model);
...
)
{
    ...
    if (probability_of_death)
        return probability_of_death(agent, *this, model);
...
)
{
    ...
    if (incubation)
        return incubation(agent, *this, model);
...
// (already included include/epiworld/model-bones.hpp)
// (already included include/epiworld/agent-meat-virus-sampling.hpp)
// (already included include/epiworld/config.hpp)
    ...
    )
{
    ...
    this->add_state("Susceptible", surveillance_update_susceptible);
    this->add_state("Latent", surveillance_update_exposed);
    this->add_state("Symptomatic", surveillance_update_exposed);
    this->add_state("Symptomatic isolated", surveillance_update_exposed);
    this->add_state("Asymptomatic", surveillance_update_exposed);
    this->add_state("Asymptomatic isolated", surveillance_update_exposed);
    this->add_state("Recovered");
    this->add_state("Removed");

    // General model parameters
    this->add_param(latent_period, "Latent period");
    this->add_param(infect_period, "Infect period");
    this->add_param(prob_symptoms, "Prob of symptoms");
    this->add_param(surveillance_prob, "Surveilance prob.");
    this->add_param(efficacy_vax, "Vax efficacy");
    this->add_param(prop_vax_redux_transm, "Vax redux transmission");
    this->add_param(prob_transmission, "Prob of transmission");
    this->add_param(prob_death, "Prob. death");
    this->add_param(prob_noreinfect, "Prob. no reinfect");
    ...
    this->add_virus(covid);

    this->set_user_data({"nsampled", "ndetected", "ndetected_asympt", "nasymptomatic"});
    this->add_globalevent(surveillance_program, "Surveilance program", -1);
    ...
    )
{
    ...
    this->set_name("Susceptible-Infected-Removed (SIR) (connected)");
    ...
    )
{
    ...
    // Preparing the virus -------------------------------------------
    Virus<TSeq> virus(vname, prevalence, true);
    virus.set_state(EXPOSED, REMOVED, DECEASED);
    ...
    this->set_name("Susceptible-Exposed-Infected-Removed-Deceased (SEIRD) (connected)");
...
////////////////////////////////////////////////////////////////////////////////
 ...
 Start of -include/epiworld/models/measlesschool.hpp-
...
////////////////////////////////////////////////////////////////////////////////
 ...
 End of -include/epiworld/models/measlesschool.hpp-
...
////////////////////////////////////////////////////////////////////////////////
 ...
 Start of -include/epiworld/models/measlesmixing.hpp-
...
public:
    ...
    std::vector< size_t > get_agent_quarantine_triggered() const
    {
        std::vector< size_t > res(this->size());
        for (size_t i = 0u; i < this->size(); ++i)
            res[i] = _quarantine->get_quarantine_status(i);
        return res;
    };
    ...
    std::vector< bool > get_quarantine_willingness() const
    {
        std::vector< bool > res(this->size());
        for (size_t i = 0u; i < this->size(); ++i)
            res[i] = _quarantine->is_quarantine_willing(i);
        return res;
    };
    ...
    std::vector< bool > get_isolation_willingness() const
    {
        std::vector< bool > res(this->size());
        for (size_t i = 0u; i < this->size(); ++i)
            res[i] = _quarantine->is_isolation_willing(i);
        return res;
    };
...
inline void ModelMeaslesMixing<TSeq>::reset()
{
    ...
    for (size_t idx = 0; idx < this->size(); ++idx)
    {
        ...
        _quarantine->set_isolation_willing(
            idx, this->runif() < this->par("Isolation willingness")
        );
    }
...
inline void ModelMeaslesMixing<TSeq>::m_update_susceptible(
    ...
    for (size_t n = 0u; n < ndraws; ++n)
    {
        ...
        // Adding the current agent to the tracked interactions
        m_down->_quarantine->add_contact(neighbor.get_id(), p->get_id(), m->today());
...
inline void ModelMeaslesMixing<TSeq>::m_update_rash(
    ...
    {
        model->_quarantine->trigger_quarantine(p->get_id());
        model->_quarantine->set_exposure_date(
            p->get_id(), model->day_rash_onset[p->get_id()]
        );
        detected = true;
    }
    ...
    else if (detected)
    {
        ...
        // isolation.
        p->change_state(*m, ISOLATED);
        model->_quarantine->set_day_flagged(p->get_id(), m->today());
    }
...
////////////////////////////////////////////////////////////////////////////////
 ...
 End of -include/epiworld/models/measlesmixing.hpp-
 ...
 * Susceptible → Latent → Prodromal → Rash → Recovered
 * 
...
inline void ModelMeaslesMixingRiskQuarantine<TSeq>::m_quarantine_process() {
    ...
    for (size_t i = 0u; i < agents_triggered_contact_tracing_size; ++i)
    {
        ...
        size_t n_contacts = _quarantine->get_contact_tracing().get_n_contacts(agent_i_idx);
        if (n_contacts > EPI_MAX_TRACKING)
            n_contacts = EPI_MAX_TRACKING;
        ...
        for (size_t contact_j_idx = 0u; contact_j_idx < n_contacts; ++contact_j_idx)
        {
            ...
            auto [contact_id, contact_date] = _quarantine->get_contact_tracing().get_contact(
    ...
    for (auto & agent: this->get_agents())
    {
        ...
        if (_quarantine->is_quarantine_willing(agent.get_id()) == false)
            continue;

        // Setting the day flagged to today
        _quarantine->set_day_flagged(agent.get_id(), this->today());
        ...
        {
            if (_quarantine->is_isolation_willing(agent.get_id()))
                agent.change_state(*this, ISOLATED);
        }
        else
            throw std::logic_error(
    ...
    // SUSCEPTIBLE, LATENT, PRODROMAL, or RASH, they should be in
    // RISK_HIGH
