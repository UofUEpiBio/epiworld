...
EPIWORLD_TEST_CASE("SEIRMixing", "[SEIR-mixing]") {
    std::vector<double> contact_matrix =
        {0.8, 0.1, 0.05, 0.15, 0.8, 0.25, 0.05, 0.1, .7};
    ...
    epimodels::ModelSEIRMixing<> model_1(
        ...
        static_cast<double>(n_infected)
            / n_agents, // epiworld_double prevalence,
    ...
    auto saver =
        [&transitions, &R0s, n_infected](size_t n, Model<>* m) -> void {
    ...
    double R0_expected = model_1("Contact rate") * model_1("Prob. Transmission")
        / model_1("Prob. Recovery");

    std::cout << "R0 in ModelSEIRMixing" << std::endl;
    std::cout << "observed: " << R0_observed << " vs expected: " << R0_expected
              << std::endl;
