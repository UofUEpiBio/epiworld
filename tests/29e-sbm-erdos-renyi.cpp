...
EPIWORLD_TEST_CASE("SBM between-block edge count", "[sbm-er]") {
    ...
    // Between-block edge sampling uses uniform pair selection, so
    // the expected number of unique edges between blocks g and h is:
    //   E[unique] = n_g * n_h * (1 - exp(-p_gh))
    // where p_gh = M(g,h) / n_h.
    //
    ...
    // Expected unique between-block edges
    double expected_unique = static_cast<double>(n_g) *
    ...
    std::cout << "  Expected unique edges: " << expected_unique
              << std::endl;
    std::cout << "  Observed avg edges:    " << avg_edges << std::endl;

    // The between-block sampling is uniform, so the analytical
    // expected value should be accurate. Tolerance of 15 edges.
    REQUIRE(std::abs(avg_edges - expected_unique) < 15.0);
