...
EPIWORLD_TEST_CASE("SIR dist", "[SIR-dist]") {
    ...
    auto tmat_0_not_norm =
        model_0.get_db().get_transition_probability(false, false);
    auto tmat_1_not_norm =
        model_1.get_db().get_transition_probability(false, false);
    ...
    REQUIRE_THAT(
        tmat_0_not_norm,
        Catch::Approx(tmat_not_norm_expected).margin(0.025)
    );
...
// Checking that the history matches the expected values
#ifdef CATCH_CONFIG_MAIN
    REQUIRE_FALSE(((h_0[0u] != 4950) || (h_0[1u] != 100) || (h_0[2u] != 4950)));
#endif

    // Trying out the SEIR model now -------------------------------------------
    epimodels::ModelSEIR<> model_2("a virus", 0.01, .5, 7.0, .1);
...
#ifdef CATCH_CONFIG_MAIN
    REQUIRE_FALSE(
        ((h_0[0u] != 4950) || (h_0[1u] != 70) || (h_0[2u] != 30)
         || (h_0[3u] != 4950))
    );
#endif

    // Trying SIRCONN ----------------------------------------------------------
    epimodels::ModelSIRCONN<> model_3("a virus", 10000, 0.01, 2, .5, .3);

    model_3.initial_states({.5}).verbose_off().run(100, 222).print(false);
    ...
    // Trying SEIRCONN ---------------------------------------------------------
    epimodels::ModelSEIRCONN<> model_4("a virus", 10000, 0.01, 2, .5, 7, .1);

    model_4.initial_states({.3, .5}).verbose_off().run(100, 222).print(false);
    ...
    // Trying SIRD -------------------------------------------------------------
    epimodels::ModelSIRD<> model_5("a virus", 0.01, .9, .3, .1);
...
#ifdef CATCH_CONFIG_MAIN
    REQUIRE_FALSE(
        (moreless(h_0[0u], 4700) || moreless(h_0[1u], 100)
         || moreless(h_0[2u], 4705) || moreless(h_0[3u], 495))
    );
#endif

    // Trying SEIRD -------------------------------------------------------------
    epimodels::ModelSEIRD<> model_6("a virus", 0.1, .9, 7, .3, .1);
...
#ifdef CATCH_CONFIG_MAIN
    REQUIRE_FALSE(
        (moreless(h_0[0u], 7200) || moreless(h_0[1u], 500)
         || moreless(h_0[2u], 500) || moreless(h_0[3u], 900)
         || moreless(h_0[4u], 900))
    );
#endif

    // Trying SEIRDCONN --------------------------------------------------------
    epimodels::ModelSEIRDCONN<>
        model_7("a virus", 10000, 0.1, 4, .9, 7, .3, .1);
