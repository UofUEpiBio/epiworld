...
EPIWORLD_TEST_CASE("Distribution funs", "[DistFuns]") {
    epimodels::ModelSIRCONN<>
        model_0("a virus", 10000u, 0.01, 4.0, 1.0, 1.0 / 10000.0);

    Tool<> tool("vax");
    tool.set_susceptibility_reduction(0.5);
    tool.set_distribution(distribute_tool_randomly(0.1, true));
    ...
    epimodels::ModelSIRCONN<>
        model_1("a virus", 10000u, 0.01, 4.0, 1.0, 1.0 / 10000.0);

    model_1.get_virus(0).set_distribution(distribute_virus_to_set<>(got_it));

    model_1.add_tool(tool);
    model_1.get_tool(0).set_distribution(distribute_tool_to_set<>(got_tool));
