...
#define mat(i, j) avg_transitions[j * n_states + i]
    double p_recovered =
        1.0 - (1.0 / model_0("Rash period") + model_0("Hospitalization rate"));
    ...
    REQUIRE_FALSE(moreless(mat(7, 9), 1.0 / model_0("Incubation period"), 0.05)
    );

    // From quarantine prodromal to isolated (rash cases are dected immediately)
    REQUIRE_FALSE(moreless(mat(9, 4), 1.0 / model_0("Prodromal period"), 0.05));

    // Transition to hospitalized
    REQUIRE_FALSE(
        moreless(mat(3, 6) + mat(3, 11), model_0("Hospitalization rate"), 0.05)
    );
...
#endif
    // Reproductive number
    std::cout << "Effective Rt (lower): " << R0_observed << " (R0 ~" << R0_theo
              << ")" << std::endl;

    // Transition to prodromal
    std::cout << "Transition to prodromal: " << mat(1, 2) + mat(1, 9)
              << " (expected ~" << 1.0 / model_0("Incubation period") << ")"
              << std::endl;

    // Transition to rash
    std::cout << "Transition to rash: " << mat(2, 3) + mat(2, 4)
              << " (expected ~" << 1.0 / model_0("Prodromal period") << ")"
              << std::endl;

    // Transition to isolated
    std::cout << "Transition to isolated: " << mat(3, 4) + mat(3, 5) + mat(3, 6)
              << " (expected ~" << 1.0 / model_0("Days undetected") << ")"
              << std::endl;

    // Transition from Quarantine Exposed to Quarantined Prodromal
    std::cout << "Transition from Q. Exposed to Q. Prodromal: " << mat(7, 9)
              << " (expected ~" << 1.0 / model_0("Incubation period") << ")"
              << std::endl;

    // From quarantine prodromal to isolated (rash cases are dected immediately)
    std::cout << "Transition from Q. Prodromal to isolated: " << mat(9, 4)
              << " (expected ~" << 1.0 / model_0("Prodromal period") << ")"
              << std::endl;
    ...
    std::cout << "Transition to hospitalized (rash): " << mat(4, 11)
              << " (expected ~" << model_0("Hospitalization rate") << ")"
              << std::endl;

    // Transition to recovered
    std::cout << "Transition to recovered (prodromal): "
              << mat(3, 5) + mat(3, 12) << " (expected ~" << p_recovered << ")"
              << std::endl;
    std::cout << "Transition to recovered (rash): " << mat(4, 5) + mat(4, 12)
              << " (expected ~" << p_recovered << ")" << std::endl;

    // Transition from hospitalized to recovered
    std::cout << "Transition from hospitalized to recovered: " << mat(11, 12)
              << " (expected ~" << 1.0 / model_0("Hospitalization period")
              << ")" << std::endl;
#undef mat
#ifndef CATCH_CONFIG_MAIN
    return 0;
