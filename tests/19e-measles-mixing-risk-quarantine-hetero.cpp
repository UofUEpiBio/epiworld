...
/**
 * @brief Test heterogeneous contact rates in ModelMeaslesMixingRiskQuarantine
 *
 ...
 * - Quarantine and detection are disabled to isolate the contact-rate effect
 *
 ...
 */
EPIWORLD_TEST_CASE(
    "MeaslesMixingRiskQuarantine heterogeneous contact rates",
    "[MeaslesMixingRiskQuarantine-hetero]"
) {
    ...
    measles::ModelMeaslesMixingRiskQuarantine<> model(
        ...
        -1.0,              // days_undetected (< 0 = never detected → no quarantine)
        -1,                // quarantine_period_high (disabled)
        -1,                // quarantine_period_medium (disabled)
        -1,                // quarantine_period_low (disabled)
        0.0,               // quarantine_willingness
        0.0,               // isolation_willingness
        -1,                // isolation_period (disabled)
        0.0,               // prop_vaccinated
        0.0                // detection_rate_quarantine
    );
    ...
    // Print results
    std::cout << "========================================================" << std::endl;
    std::cout << "Heterogeneous contact rates test (ModelMeaslesMixingRiskQuarantine)" << std::endl;
