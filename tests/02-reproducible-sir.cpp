...
EPIWORLD_TEST_CASE("SIR parallel", "[SIR parallel]") {
    ...
    for (auto f : files) {
        for (size_t i = 0u; i < 4u; ++i) {
            std::string file_0 = "02-reproducible-sir-saves/main_out_"
                + std::to_string(i) + "_" + f + +".csv";
            std::string file_1 = "02-reproducible-sir-saves/main_out_pll_"
                + std::to_string(i) + "_" + f + +".csv";
            ...
            if (file0 != file1) {
                std::cout << "Files " << file_0 << " and " << file_1
