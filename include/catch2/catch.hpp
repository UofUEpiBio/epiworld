...
namespace Catch {
    ...
    class StringRef {
      ...
      public: // substrings and searches
        ...
        auto
        substr(size_type start, size_type length) const noexcept -> StringRef;
    ...
    constexpr auto
    operator"" _sr(char const* rawChars, std::size_t size) noexcept
        -> StringRef {
...
operator"" _catch_sr(char const* rawChars, std::size_t size) noexcept
    ...
    #define CATCH_REC_LIST0_UD(f, userdata, x, peek, ...) \
        , \
            f(userdata, x) \
                CATCH_DEFER(CATCH_REC_NEXT(peek, CATCH_REC_LIST1_UD))( \
                    f, \
                    userdata, \
                    peek, \
                    __VA_ARGS__ \
    ...
    #define CATCH_REC_LIST1_UD(f, userdata, x, peek, ...) \
        , \
            f(userdata, x) \
                CATCH_DEFER(CATCH_REC_NEXT(peek, CATCH_REC_LIST0_UD))( \
                    f, \
                    userdata, \
                    peek, \
                    __VA_ARGS__ \
        ...
        #define INTERNAL_CATCH_MAKE_TYPE_LIST2(...) \
            INTERNAL_CATCH_EXPAND_VARGS( \
                decltype(get_wrapper< \
                         INTERNAL_CATCH_REMOVE_PARENS_GEN(__VA_ARGS__)>()) \
            )
        #define INTERNAL_CATCH_MAKE_TYPE_LIST(...) \
...
\
    ...
    #define INTERNAL_CATCH_DEFINE_SIG_TEST_METHOD1(TestName, signature) \
        template<typename TestType> \
        void INTERNAL_CATCH_MAKE_NAMESPACE(TestName \
...
namespace Catch {
    ...
    #if defined(CATCH_CONFIG_DISABLE)
        #define INTERNAL_CATCH_TRANSLATE_EXCEPTION_NO_REG( \
            translatorName, \
            signature \
        ) \
...
namespace Catch {
    namespace Matchers {
        namespace Impl {
            ...
            template<typename T>
            MatchAllOf<T> MatcherBase<T>::operator&&(MatcherBase const& other
            ) const {
                return MatchAllOf<T>() && *this && other;
            }

            template<typename T>
            MatchAnyOf<T> MatcherBase<T>::operator||(MatcherBase const& other
            ) const {
    ...
    namespace Matchers {
        ...
        namespace Vector {
            ...
            struct UnorderedEqualsMatcher:
                ...
                bool match(std::vector<T, AllocMatch> const& vec
                ) const override {
        ...
        class RangeGenerator final: public IGenerator<T> {
          ...
          public:
            RangeGenerator(T const& start, T const& end, T const& step) :
                ...
                assert(
                    m_current != m_end && "Range start and end cannot be equal"
                );
...
namespace Catch {
    ...
    struct IReporterFactory {
        ...
        virtual IStreamingReporterPtr create(ReporterConfig const& config
        ) const = 0;
...
namespace Catch {
    ...
    class ReporterRegistrar {
        class ReporterFactory: public IReporterFactory {
            IStreamingReporterPtr create(ReporterConfig const& config
            ) const override {
...
namespace Catch {
    ...
    struct CompactReporter: StreamingReporterBase<CompactReporter> {
        ...
        // start catch_reporter_console.h
            ...
            ) // Not all labels are EXPLICITLY handled in switch \
...
namespace Catch {
    namespace Benchmark {
        struct Benchmark {
            ...
            prepare(const IConfig& cfg, Environment<FloatDuration<Clock>> env)
                const {
                auto min_time =
            ...
            void run() {
                ...
                CATCH_CATCH_ALL {
                    if (translateActiveException()
                        != Detail::
                            benchmarkErrorMsg) // benchmark errors have been reported, otherwise rethrow.
                        std::rethrow_exception(std::current_exception());
                }
            }
...
namespace Catch {
    ...
    void AssertionHandler::handleExceptionThrownAsExpected() {
        m_resultCapture
            .handleNonExpr(m_assertionInfo, ResultWas::Ok, m_reaction);
    }
            ...
            class Column {
              ...
              public:
                class iterator {
                    ...
                    auto addIndentAndSuffix(std::string const& plain
                    ) const -> std::string {
...
namespace Catch {
    namespace clara {
        namespace detail {
            ...
            template<typename DerivedT>
            template<typename T>
            auto ComposableParserImpl<DerivedT>::operator|(T const& other
            ) const -> Parser {
...
namespace Catch {
    ...
    bool SourceLineInfo::operator==(SourceLineInfo const& other
    ) const noexcept {
...
namespace Catch {
    namespace Matchers {
        namespace Exception {
            ...
            bool ExceptionMessageMatcher::match(std::exception const& ex
            ) const {
...
namespace Catch {
    namespace Matchers {
        ...
        namespace StdString {
            ...
            std::string CasedString::adjustString(std::string const& str
            ) const {
            ...
            switch (c) {
                ...
                case ',':
                    if (start != pos && openings.empty()) {
                        m_messages
                            .emplace_back(macroName, lineInfo, resultType);
...
namespace Catch {
    ...
    void RunContext::handleFatalErrorCondition(StringRef message) {
        ...
        assertions.failed = 1;
        SectionStats
            testCaseSectionStats(testCaseSection, assertions, 0, false);
        ...
        class TestGroup {
          public:
            explicit TestGroup(std::shared_ptr<Config> const& config) :
                ...
                m_matches = m_config->testSpec().matchesByFilter(
                    allTestCases,
                    *m_config
                );
...
namespace Catch {
    ...
    std::string
    TagAliasRegistry::expandAliases(std::string const& unexpandedTestSpec
    ) const {
...
namespace Catch {
    ...
    ///////////////////////////////////////////////////////////////////////////
    TestInvokerAsFunction::TestInvokerAsFunction(void (*testAsFunction)()
    ) noexcept :
    ...
    namespace TestCaseTracking {
        ...
        void SectionTracker::addInitialFilters(
            ...
            if (!filters.empty()) {
                ...
                m_filters
                    .insert(m_filters.end(), filters.begin(), filters.end());
            }
        }
...
namespace Catch {
    ...
    bool TestSpec::ExcludedPattern::matches(TestCaseInfo const& testCase
    ) const {
...
namespace Catch {
    ...
    std::string WildcardPattern::normaliseString(std::string const& str) const {
        return trim(
            m_caseSensitivity == CaseSensitive::No ? toLower(str) : str
        );
    }
} // namespace Catch
...
extern "C" int CATCH_INTERNAL_CDECL
