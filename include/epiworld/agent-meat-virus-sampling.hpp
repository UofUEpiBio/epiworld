 ...
 * that it will create a function that supports excluding states from the
 * sampling frame. For example, individuals who have acquired a virus can be
 * excluded if in incubation state.
 *
 * @tparam TSeq
 * @param exclude unsigned vector of states that need to be excluded from the
 * sampling
...
make_update_susceptible(std::vector<epiworld_fast_uint> exclude = {}) {
  ...
  if (exclude.size() == 0u) {
    ...
    std::function<void(Agent<TSeq> *, Model<TSeq> *)> sampler =
      ...
      if (p->get_virus() != nullptr)
        throw std::logic_error(
            std::string("Using the -default_update_susceptible- on agents WITH "
    ...
    std::function<void(Agent<TSeq> *, Model<TSeq> *)> sampler =
        [exclude_agent_bool, exclude_agent_bool_idx](Agent<TSeq> *p,
                                                     Model<TSeq> *m) -> void {
      ...
      if (exclude_agent_bool->size() == 0u) {
        ...
        for (auto s : *exclude_agent_bool_idx) {
          if (s >= exclude_agent_bool->size())
            throw std::logic_error(std::string("You are trying to exclude a "
                                               "state that is out of range: ") +
                                   std::to_string(s) +
    ...
    std::function<Virus<TSeq> *(Agent<TSeq> *, Model<TSeq> *)> res =
      ...
      for (auto &neighbor : p->get_neighbors()) {
        ...
        if (nviruses_tmp >= static_cast<int>(m->array_virus_tmp.size()))
          throw std::logic_error("Trying to add an extra element to a temporal "
                                 "array outside of the range.");
#endif
...
inline Virus<TSeq> *sample_virus_single(Agent<TSeq> *p, Model<TSeq> *m) {
  ...
  for (auto &neighbor : p->get_neighbors()) {
    ...
    if ((m->array_double_tmp[nviruses_tmp - 1] < 0.0) |
      ...
      printf_epiworld("[epi-debug] Agent %i's virus %i has transmission prob "
                      "outside of [0, 1]: %.4f!\n",
