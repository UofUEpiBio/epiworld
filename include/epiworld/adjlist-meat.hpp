...
inline AdjList::AdjList(const std::vector<int> &source,
                        const std::vector<int> &target, int size, bool directed)
    : directed(directed) {
  ...
  for (int m = 0; m < static_cast<int>(source.size()); ++m) {
    ...
    if (i > max_id)
      throw std::range_error("The source[" + std::to_string(m) +
                             "] = " + std::to_string(i) +
                             " is above the max_id " + std::to_string(max_id));

    if (j > max_id)
      throw std::range_error("The target[" + std::to_string(m) +
                             "] = " + std::to_string(j) +
                             " is above the max_id " + std::to_string(max_id));
...
inline AdjList::AdjList(AdjList &&a) noexcept
    : dat(std::move(a.dat)), directed(a.directed), N(a.N), E(a.E) {}

inline AdjList::AdjList(const AdjList &a)
    : dat(a.dat), directed(a.directed), N(a.N), E(a.E) {}
...
inline void AdjList::read_edgelist(std::string fn, int size, int skip,
                                   bool directed) {
  ...
  while (!filei.eof()) {
    ...
    if (i > max_id)
      throw std::range_error("The source[" + std::to_string(linenum) +
                             "] = " + std::to_string(i) +
                             " is above the max_id " + std::to_string(max_id));

    if (j > max_id)
      throw std::range_error("The target[" + std::to_string(linenum) +
                             "] = " + std::to_string(j) +
                             " is above the max_id " + std::to_string(max_id));
...
inline std::map<int, int> AdjList::operator()(epiworld_fast_uint i) const {
  ...
  if (i >= N)
    throw std::range_error("The vertex id " + std::to_string(i) +
...
inline void AdjList::print(epiworld_fast_uint limit) const {
  ...
  if (limit < dat.size()) {
    printf_epiworld("  (... skipping %i records ...)\n",
                    static_cast<int>(dat.size() - limit));
  }
}

inline size_t AdjList::vcount() const { return N; }

inline size_t AdjList::ecount() const { return E; }
