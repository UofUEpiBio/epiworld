...
inline void default_add_virus(Event<TSeq> &a, Model<TSeq> *m) {
  ...
  m->get_db().record_transmission(v->get_agent() ? v->get_agent()->get_id()
                                                 : -1,
                                  p->get_id(), v->get_id(), v->get_date());
  ...
  if ((a.new_state != -99) && (static_cast<int>(p->state) != a.new_state)) {
    ...
    for (size_t i = 0u; i < p->n_tools; ++i)
      db.update_tool(p->tools[i]->get_id(), p->state, a.new_state);
  }

// Lastly, we increase the daily count of the virus
#ifdef EPI_DEBUG
  m->get_db()
      .today_virus.at(p->virus->get_id())
      .at(a.new_state != -99 ? a.new_state : p->state)++;
#else
...
inline void default_add_tool(Event<TSeq> &a, Model<TSeq> *m) {
  ...
  if ((a.new_state != -99) && static_cast<int>(p->state) != a.new_state) {
    ...
    // because state_prev may be stale if multiple changes occurred today
    if (p->virus)
      db.update_virus(p->virus->get_id(), p->state, a.new_state);
  }
...
inline void default_rm_entity(Event<TSeq> &a, Model<TSeq> *m) {
  ...
  if (--p->n_entities > 0) {
    ...
    Entity<TSeq> *last_entity = &m->get_entity(
        p->entities[p->n_entities]); ///< Last entity of the agent
