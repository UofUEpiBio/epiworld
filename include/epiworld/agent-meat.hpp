...
template <typename TSeq> inline Agent<TSeq>::Agent() {}

template <typename TSeq>
inline Agent<TSeq>::Agent(Agent<TSeq> &&p)
    : model(p.model), neighbors(std::move(p.neighbors)),
      neighbors_locations(std::move(p.neighbors_locations)),
      n_neighbors(p.n_neighbors), entities(std::move(p.entities)),
      entities_locations(std::move(p.entities_locations)),
      n_entities(p.n_entities), state(p.state), state_prev(p.state_prev),
      state_last_changed(p.state_last_changed), id(p.id),
...
inline Agent<TSeq>::Agent(const Agent<TSeq> &p)
    : model(p.model), neighbors(nullptr), neighbors_locations(nullptr),
      n_neighbors(p.n_neighbors), entities(p.entities),
      entities_locations(p.entities_locations), n_entities(p.n_entities) {
...
inline Agent<TSeq> &Agent<TSeq>::operator=(const Agent<TSeq> &other_agent) {
  ...
  if (other_agent.n_neighbors > 0u) {
    ...
    neighbors_locations =
        new std::vector<size_t>(*other_agent.neighbors_locations);
  } else {
    neighbors = nullptr;
...
template <typename TSeq>
inline void Agent<TSeq>::add_tool(ToolPtr<TSeq> &tool, Model<TSeq> *model,
  ...
  model->events_add(this, nullptr, tool, nullptr, state_new, queue,
                    default_add_tool<TSeq>, -1, -1);
}

template <typename TSeq>
inline void Agent<TSeq>::add_tool(const Tool<TSeq> &tool, Model<TSeq> *model,
                                  epiworld_fast_int state_new,
                                  epiworld_fast_int queue) {
...
template <typename TSeq>
inline void Agent<TSeq>::set_virus(VirusPtr<TSeq> &virus, Model<TSeq> *model,
                                   epiworld_fast_int state_new,
                                   epiworld_fast_int queue) {
  ...
  model->events_add(this, virus, nullptr, nullptr, state_new, queue,
                    default_add_virus<TSeq>, -1, -1);
}

template <typename TSeq>
inline void Agent<TSeq>::set_virus(const Virus<TSeq> &virus, Model<TSeq> *model,
                                   epiworld_fast_int state_new,
                                   epiworld_fast_int queue) {
...
template <typename TSeq>
inline void Agent<TSeq>::add_entity(Entity<TSeq> &entity, Model<TSeq> *model,
                                    epiworld_fast_int state_new,
                                    epiworld_fast_int queue) {

  if (model != nullptr) {

    model->events_add(this, nullptr, nullptr, &entity, state_new, queue,
                      default_add_entity<TSeq>, -1, -1);
         ...
         // model entity
  {
    ...
    Event<TSeq> a(this, nullvirus, nulltool, &entity, state_new, queue, call,
                  -1, -1);
...
template <typename TSeq>
inline void
Agent<TSeq>::rm_tool(epiworld_fast_uint tool_idx, Model<TSeq> *model,
                     epiworld_fast_int state_new, epiworld_fast_int queue) {
  ...
  model->events_add(this, nullptr, tools[tool_idx], nullptr, state_new, queue,
                    default_rm_tool<TSeq>, -1, -1);
}

template <typename TSeq>
inline void Agent<TSeq>::rm_tool(ToolPtr<TSeq> &tool, Model<TSeq> *model,
                                 epiworld_fast_int state_new,
                                 epiworld_fast_int queue) {
...
template <typename TSeq>
inline void Agent<TSeq>::rm_virus(Model<TSeq> *model,
                                  epiworld_fast_int state_new,
                                  epiworld_fast_int queue) {
...
template <typename TSeq>
inline void
Agent<TSeq>::rm_entity(epiworld_fast_uint entity_idx, Model<TSeq> *model,
                       epiworld_fast_int state_new, epiworld_fast_int queue) {
  ...
  model->events_add(this, nullptr, nullptr, &model->get_entity(entity_idx),
                    state_new, queue, default_rm_entity<TSeq>,
                    entities_locations[entity_idx], entity_idx);
}

template <typename TSeq>
inline void Agent<TSeq>::rm_entity(Entity<TSeq> &entity, Model<TSeq> *model,
                                   epiworld_fast_int state_new,
                                   epiworld_fast_int queue) {
  ...
  if (entity_idx == -1)
    throw std::logic_error(std::string("The agent ") + std::to_string(id) +
                           std::string(" is not associated with entity \"") +
                           entity.get_name() + std::string("\"."));

  model->events_add(this, nullptr, nullptr, &model->entities[entity.get_id()],
                    state_new, queue, default_rm_entity<TSeq>,
                    entities_locations[entity_idx], entity_idx);
}
...
inline void Agent<TSeq>::rm_agent_by_virus(Model<TSeq> *model) {
  ...
  model->events_add(this, virus, nullptr, nullptr, virus->state_removed,
                    virus->queue_removed, default_rm_virus<TSeq>, -1, -1);
}

template <typename TSeq>
inline epiworld_double
Agent<TSeq>::get_susceptibility_reduction(VirusPtr<TSeq> v,
                                          Model<TSeq> *model) {
...
template <typename TSeq>
inline epiworld_double Agent<TSeq>::get_recovery_enhancer(VirusPtr<TSeq> v,
                                                          Model<TSeq> *model) {
  return model->recovery_enhancer_mixer(this, v, model);
}

template <typename TSeq>
inline epiworld_double Agent<TSeq>::get_death_reduction(VirusPtr<TSeq> v,
                                                        Model<TSeq> *model) {
  return model->death_reduction_mixer(this, v, model);
}

template <typename TSeq> inline int Agent<TSeq>::get_id() const { return id; }

template <typename TSeq> inline VirusPtr<TSeq> &Agent<TSeq>::get_virus() {
  return virus;
}
...
template <typename TSeq> inline Tools<TSeq> Agent<TSeq>::get_tools() {
...
template <typename TSeq> inline ToolPtr<TSeq> &Agent<TSeq>::get_tool(int i) {
  return tools.at(i);
}
...
template <typename TSeq> inline void Agent<TSeq>::mutate_virus() {

  virus->mutate();
}

template <typename TSeq>
inline void Agent<TSeq>::add_neighbor(Agent<TSeq> &p, bool check_source,
                                      bool check_target) {
...
template <typename TSeq>
inline void Agent<TSeq>::swap_neighbors(Agent<TSeq> &other, size_t n_this,
                                        size_t n_other) {
  ...
  if (!model->directed) {
    std::swap((*neigh_this.neighbors)[loc_this_in_neigh],
              (*neigh_other.neighbors)[loc_other_in_neigh]);

    // Changing the locations
    std::swap((*neighbors_locations)[n_this],
              (*other.neighbors_locations)[n_other]);

    std::swap((*neigh_this.neighbors_locations)[loc_this_in_neigh],
...
template <typename TSeq> inline size_t Agent<TSeq>::get_n_neighbors() const {
  return n_neighbors;
}

template <typename TSeq>
inline void Agent<TSeq>::change_state(Model<TSeq> *model,
                                      epiworld_fast_uint new_state,
                                      epiworld_fast_int queue) {

  model->events_add(this, nullptr, nullptr, nullptr, new_state, queue,
...
template <typename TSeq> inline void Agent<TSeq>::reset() {
...
template <typename TSeq> inline void Agent<TSeq>::print(bool compressed) const {

  if (compressed) {
    printf_epiworld(
        "Agent: %i, state: %s (%i), Has virus: %s, NTools: %ii NNeigh: %i\n",
        static_cast<int>(id), model->states_labels[state].c_str(),
        static_cast<int>(state),
        virus == nullptr ? std::string("no").c_str()
                         : std::string("yes").c_str(),
        static_cast<int>(n_tools), static_cast<int>(n_neighbors));
  } else {
    ...
    if (nfeats > 0) {
      ...
      printf_epiworld("This model includes features (%i): [ ",
...
template <typename TSeq> inline double &Agent<TSeq>::operator()(size_t j) {
...
template <typename TSeq> inline double Agent<TSeq>::operator()(size_t j) const {
...
template <typename TSeq> inline Entities<TSeq> Agent<TSeq>::get_entities() {
...
inline const Entity<TSeq> &Agent<TSeq>::get_entity(size_t i) const {
  if (n_entities == 0)
    throw std::range_error("Agent id " + std::to_string(id) +
                           " has no entities.");
...
template <typename TSeq> inline size_t Agent<TSeq>::get_n_entities() const {
  return n_entities;
}
...
inline bool Agent<TSeq>::operator==(const Agent<TSeq> &other) const {
  ...
  EPI_DEBUG_FAIL_AT_TRUE(n_neighbors != other.n_neighbors,
                         "Agent:: n_eighbors don't match")

  for (size_t i = 0u; i < n_neighbors; ++i) {
    EPI_DEBUG_FAIL_AT_TRUE((*neighbors)[i] != (*other.neighbors)[i],
                           "Agent:: neighbor[i] don't match")
  }

  EPI_DEBUG_FAIL_AT_TRUE(n_entities != other.n_entities,
                         "Agent:: n_entities don't match")

  for (size_t i = 0u; i < n_entities; ++i) {
    EPI_DEBUG_FAIL_AT_TRUE(entities[i] != other.entities[i],
                           "Agent:: entities[i] don't match")
  }

  EPI_DEBUG_FAIL_AT_TRUE(state != other.state, "Agent:: state don't match")

  EPI_DEBUG_FAIL_AT_TRUE(state_prev != other.state_prev,
  ...
  EPI_DEBUG_FAIL_AT_TRUE(((virus == nullptr) && (other.virus != nullptr)) ||
                             ((virus != nullptr) && (other.virus == nullptr)),
                         "Agent:: virus don't match")

  if ((virus != nullptr) && (other.virus != nullptr)) {
    EPI_DEBUG_FAIL_AT_TRUE(*virus != *other.virus,
                           "Agent:: virus doesn't match")
  }

  EPI_DEBUG_FAIL_AT_TRUE(n_tools != other.n_tools,
                         "Agent:: n_tools don't match")

  for (size_t i = 0u; i < n_tools; ++i) {

    EPI_DEBUG_FAIL_AT_TRUE(tools[i] != other.tools[i],
