...
template <typename TSeq> class Virus;

template <typename TSeq> class Viruses;

template <typename TSeq> class Viruses_const;

template <typename TSeq> class Tool;

template <typename TSeq> class Tools;

template <typename TSeq> class Tools_const;

template <typename TSeq> class Queue;

template <typename TSeq> struct Event;

template <typename TSeq> class Entity;

template <typename TSeq> class Entities;
...
public:
   ...
   */
  ///@{
  void add_tool(ToolPtr<TSeq> &tool, Model<TSeq> *model,
                epiworld_fast_int state_new = -99,
                epiworld_fast_int queue = -99);

  void add_tool(const Tool<TSeq> &tool, Model<TSeq> *model,
                epiworld_fast_int state_new = -99,
                epiworld_fast_int queue = -99);

  void set_virus(VirusPtr<TSeq> &virus, Model<TSeq> *model,
                 epiworld_fast_int state_new = -99,
                 epiworld_fast_int queue = -99);

  void set_virus(const Virus<TSeq> &virus, Model<TSeq> *model,
                 epiworld_fast_int state_new = -99,
                 epiworld_fast_int queue = -99);

  void add_entity(Entity<TSeq> &entity, Model<TSeq> *model,
                  epiworld_fast_int state_new = -99,
                  epiworld_fast_int queue = -99);

  void rm_tool(epiworld_fast_uint tool_idx, Model<TSeq> *model,
               epiworld_fast_int state_new = -99,
               epiworld_fast_int queue = -99);

  void rm_tool(ToolPtr<TSeq> &tool, Model<TSeq> *model,
               epiworld_fast_int state_new = -99,
               epiworld_fast_int queue = -99);

  void rm_virus(Model<TSeq> *model, epiworld_fast_int state_new = -99,
                epiworld_fast_int queue = -99);

  void rm_entity(epiworld_fast_uint entity_idx, Model<TSeq> *model,
                 epiworld_fast_int state_new = -99,
                 epiworld_fast_int queue = -99);

  void rm_entity(Entity<TSeq> &entity, Model<TSeq> *model,
                 epiworld_fast_int state_new = -99,
                 epiworld_fast_int queue = -99);
  ...
  void mutate_virus();
  void add_neighbor(Agent<TSeq> &p, bool check_source = true,
  ...
  void change_state(Model<TSeq> *model, epiworld_fast_uint new_state,
